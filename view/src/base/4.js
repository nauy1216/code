/**
小数
1、数字类型只有number类型，number类型相当于其他强类型语言中的double类型(双精度浮点型)，不区分浮点型和整数型。

2、由于Js的所有数字类型都是双精度浮点型(64位)采用 IEEE754 标准
64位二进制数表示一个number数字
...
3-2、小数转二进制
由于也需要转化为指数形式，例如 1/2 = 1 * 2^-1, 1/4 = 1 * 2^-2,
所以小数的转化二进制过程是通过判断小数是不是满 1/2，1/4，8/1以此类推，换成数学公式就是 乘二取整法
