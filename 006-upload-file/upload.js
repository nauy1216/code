...
async function uploadFile () {
  ...
  let blockLength = Math.ceil(fileSize / blockSize); // 需要分片的数量
  let start = 0, end = blockSize; // 当前分片数据的起始位置、结束位置
  
  // 对文件进行切割
  let blockContent = selectedFile.slice(start, end);

  let formData = new FormData();
  formData.set('fileId', selectedFileHash); // 文件的唯一标识
  formData.set('fileName', fileName);       // 文件名
  formData.set('blockLength', blockLength); // 需要分片的数量
  formData.set('blockIndex', 0);            // 当前分片的索引
  formData.set('blockContent', blockContent); // 当前分片的内容
