...
2-2、强缓存
在加载资源时，浏览器先判断缓存是否命中，如果命中，直接从缓存中读取静态资源，不会向浏览器确认缓存是否有效。
这种方式的好处是可以减少一次 http 请求。
...
3-1、Expires
Expires 是 http1.0 提出的一个表示资源过期时间的 header，它描述的是一个绝对时间，
由服务器返回，用 GMT 格式的字符串表示。缺点是由服务器端返回的一个绝对时间，
如果客户端和服务器时间相差较大，或者处于不同时区，那么误差就会很大。
...
要配合Cache-Control 使用。
配置Cache-Control: no-cache, 表示不缓存(相当于过期时间为0，每次都会询问服务器)
Last-Modified(在响应头)：标示这个响应资源的最后修改时间。
If-Modified-Since(在请求头)：当缓存过期且发现资源具有 Last-Modified 声明，则向服务器发送请求，并带上 If-Modified-Since，表示请求时间。
服务器收到请求后，将被请求资源的最后修改时间做对比，若最后修改时间较新，则返回 304，若最后修改时间较旧，则返回 200。
...
2、application cache的使用会修改文档的加载过程：
...
1、对于 Service Worker，了解过 Web Worker 的同学可能会比较好理解。它和 Web Worker 相比，有相同的点，也有不同的地方。
1-1、相同：
（1）Service Worker工作在worker context中，是没有访问 DOM 的权限的，所以我们无法在 Service Worker 中获取 DOM 节点，也无法在其中操作 DOM 元素；
